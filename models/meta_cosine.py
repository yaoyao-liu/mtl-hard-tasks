...
def MakeMetaModel():
    ...
    class MetaModel(Models):
        ...
        def forward_base(self, inp, fc_weights):
            fc_w5 = fc_weights['w5']
            [dim0, dim1] = fc_w5.get_shape().as_list()[0:2]
            fc_w5_norm = tf.sqrt(tf.reduce_sum(tf.square(fc_w5), axis=0))
            fc_w5_norm = tf.tile(tf.expand_dims(fc_w5_norm, axis=0), multiples=[dim0, 1])
            fc_w5_normed = fc_w5 / (fc_w5_norm + 0.00001)

            [idim0, idim1] = inp.get_shape().as_list()[0:2]
            inp_norm = tf.sqrt(tf.reduce_sum(tf.square(inp), axis=1))
            inp_norm = tf.tile(tf.expand_dims(inp_norm, axis=1), multiples=[1, idim1])
            inp_normed = inp / (inp_norm + 0.00001)
            
            net = 10.0 * tf.matmul(inp_normed, fc_w5_normed)
            return net
