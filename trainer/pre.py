...
class PreTrainer:
    def __init__(self):
        ...
        pretrain_val_input, pretrain_val_label = pre_train_data_generator.make_data_tensor(is_val=True)
        pre_train_val_input_tensors = {'pretrain_input': pretrain_val_input, 'pretrain_label': pretrain_val_label}

        # Build Pre-train Model
        self.model = MakePreModel()
        self.model.construct_pretrain_model(input_tensors=pre_train_input_tensors)
        self.model.construct_pretrain_model(input_tensors=pre_train_val_input_tensors, is_val=True)
    ...
    def pre_train(self):
        ...
        for itr in trange(pretrain_iterations):
            ...
            if itr % 1000 == 0:
                val_acc_list = []
                for the_val_idx in range(100):
                    feed_dict = {self.model.pretrain_lr: 0.0}
                    input_tensors = self.model.pretrain_task_accuracy_val
                    result = self.sess.run(input_tensors, feed_dict)
                    val_acc_list.append(result)

                print('Val Acc:' + str(np.mean(val_acc_list)))
